...
def load_files(directory):
    ...
    for filenam in os.listdir(directory):
        with open(os.path.join(directory, filenam), encoding = "utf-8") as outfile:
            dictionary[filenam] = outfile.read()
...
def tokenize(document):
    ...
    final_list = [x for x in token if x not in string.punctuation and x not in nltk.corpus.stopwords.words("english")]
    return final_list
...
def compute_idfs(documents):
    ...
    for word in unique_words:
        ...
        idf_dictionary[word] = math.log(doc_len/count)
